...
Type objective_function<Type>::operator() ()
{
  ...
  PARAMETER_VECTOR(yjlg);
  PARAMETER_VECTOR(roclg);
  ...
  */
  ...
  vector<Type> yj = invlogit(yjlg);
  vector<Type> roc = invlogit(roclg);
  vector<Type> prev = invlogit(prevlg);
  const Type one = static_cast<Type>(1.0);

  const vector<Type> se = one - (roc*yj);
  REPORT(se);
  const vector<Type> sp = one - ((one-roc)*yj);
  // Simplified but for some reason doesn't compile:
  // const vector<Type> sp = yj*(roc-one) + one:
  REPORT(sp);
