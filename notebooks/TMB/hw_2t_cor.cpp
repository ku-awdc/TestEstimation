...
template<class Type>
Type cor_to_cov(const Type cor, const Type v1, const Type v2)
{
  const Type one = static_cast<Type>(1.0);
  const Type min = (v1-one) * (one-v2);
  const Type max = std::min(v1, v2) - (v1*v2);
    
  // Type cov = cor * ()
  // (se[1]-1)*(1-se[3]) , min(se[1],se[3]) - se[1]*se[3] 
}
...
Type objective_function<Type>::operator() ()
{
  ...
  PARAMETER_VECTOR(corse);
  PARAMETER_VECTOR(corsp);
